...
async function getOrCreateUser(user) {
  ...
  if (!snap.exists()) {
    await setDoc(ref, {
      ...
      photo: user.photoURL, plan: "free", createdAt: serverTimestamp(),
...
function LoginScreen({ onLogin }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      p.style.cssText = `left:${Math.random()*100}%;top:${Math.random()*100}%;width:${1+Math.random()*2}px;height:${1+Math.random()*2}px;background:${colors[Math.floor(Math.random()*colors.length)]};--dx:${Math.cos(rad)*dist}px;--dy:${Math.sin(rad)*dist}px;animation-duration:${5+Math.random()*10}s;animation-delay:${-Math.random()*15}s;box-shadow:0 0 4px currentColor;`;
  ...
  return (
    <>
      <style>{`
        ...
        .rp-login-root{--cyan:#00d4ff;--purple:#8b5cf6;--pink:#ff2d78;--card-bg:rgba(5,5,20,0.85);position:fixed;inset:0;z-index:9999;font-family:'Rajdhani',sans-serif;background:#000;color:#fff;overflow:hidden}
        ...
        .rp-fi-c{background:rgba(0,212,255,.15)}.rp-fi-p{background:rgba(139,92,246,.15)}.rp-fi-q{background:rgba(255,45,120,.15)}
        ...
        .rp-tdot-c{background:#00d4ff;box-shadow:0 0 6px #00d4ff}.rp-tdot-p{background:#8b5cf6;box-shadow:0 0 6px #8b5cf6}.rp-tdot-q{background:#ff2d78;box-shadow:0 0 6px #ff2d78}
        ...
        @media(max-width:768px){.rp-page{flex-direction:column;height:auto;min-height:100vh;overflow-y:auto}.rp-left{padding:50px 30px 30px}.rp-left::after{display:none}.rp-logo-img{width:130px;height:130px}.rp-brand-name{font-size:1.4rem}.rp-stats{gap:20px}.rp-features{max-width:100%}.rp-right{padding:24px 24px 50px}.rp-card{padding:36px 28px}.rp-login-root{overflow-y:auto}}
      `}</style>
      <div className="rp-login-root">
        <div className="rp-bg"/><div className="rp-grid"/>
...
export default function App() {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      // ── Build correct endpoint + body ──────────────────
      // Tools that need a file → use FormData (multipart)
      // Text-only tools → use JSON
      ...
      if (needsFile && inputFile) {
        // ✅ Send as multipart/form-data — backend uses multer to receive "image" field
        const fd = new FormData();
        fd.append("image", inputFile); // ← key must be "image" — matches upload.single("image") in backend
        if (activeTool.id === "image-to-image") fd.append("prompt", fullPrompt);
        if (activeTool.id === "image-to-video") fd.append("prompt", fullPrompt);
        fetchOptions = { method: "POST", body: fd }; // no Content-Type header — browser sets it with boundary
      } else {
        // ✅ Text-only — JSON body
  ...
  const loadHistory = useCallback(async () => {
    ...
    try { const h = await fetchHistory(user.uid); setHistory(h); }
  ...
  const download = (url, ext = "png") => {
    ...
    a.download = `rp-vision-${Date.now()}.${ext}`; a.click();
  };
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        :root{--bg:#03030a;--panel:#08080f;--card:#0e0e1a;--card2:#131320;--border:rgba(255,255,255,0.05);--border2:rgba(255,255,255,0.1);--border3:rgba(255,255,255,0.15);--accent:#e8c14a;--accent2:#f5d97a;--accent-dim:rgba(232,193,74,0.1);--accent-glow:rgba(232,193,74,0.25);--green:#2ecc71;--red:#e74c3c;--text:#eeeef5;--muted:#44445a;--muted2:#7777a0;--sidebar:280px}
        ...
        textarea{width:100%;background:var(--card);border:1.5px solid var(--border2);border-radius:12px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:13.5px;font-weight:300;line-height:1.7;padding:12px 13px;outline:none;transition:border-color .2s,box-shadow .2s;resize:none;min-height:100px;padding-bottom:34px}
        ...
        .upload-drop-zone{border:2px dashed rgba(232,193,74,0.3);border-radius:14px;padding:32px 20px;text-align:center;cursor:pointer;transition:all .2s;background:rgba(232,193,74,0.03);display:flex;flex-direction:column;align-items:center;gap:8px}
        .upload-drop-zone:hover{border-color:var(--accent);background:var(--accent-dim);transform:translateY(-1px)}
        .upload-drop-icon{width:48px;height:48px;border-radius:50%;background:var(--accent-dim);border:1.5px solid rgba(232,193,74,0.3);display:flex;align-items:center;justify-content:center;font-size:22px;color:var(--accent);font-weight:300;line-height:1}
        ...
        @media(max-width:767px){.app{position:relative}.sidebar{position:fixed;top:0;left:0;bottom:0;z-index:100;transform:translateX(-100%);transition:transform .3s ease;width:280px;min-width:280px}.sidebar.open{transform:translateX(0);animation:slideIn .3s ease}.mobile-overlay{display:block;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:99;backdrop-filter:blur(2px)}.mobile-topbar{display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid var(--border);background:var(--panel);flex-shrink:0}.hamburger{background:none;border:1px solid var(--border2);border-radius:8px;color:var(--text);font-size:18px;padding:6px 10px;cursor:pointer}.main{overflow:hidden}.main-topbar{display:none}.workspace{flex-direction:column;overflow-y:auto;-webkit-overflow-scrolling:touch}.controls{width:100%;min-width:unset;border-right:none;border-bottom:1px solid var(--border);overflow-y:visible;padding:16px}.canvas{padding:16px;justify-content:flex-start;min-height:400px}.history-view{padding:16px}.profile-view{padding:16px}}
      `}</style>
            ...
            <div className="workspace">
              <div className="controls">
                    ...
                    <div className="style-grid">
                      {STYLES.map((s,i) => (
                        <div key={i} className={"style-pill"+(style===i?" active":"")}
                          onClick={() => setStyle(style===i ? null : i)}>{s.label}</div>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <button className="gen-btn" disabled={loading||creditsLeft<activeTool.credits} onClick={generate}>
                  {loading ? <><Spinner/>&nbsp;Generating...</> : <>{activeTool.icon}&nbsp;Generate <span className="gen-btn-credits">{activeTool.credits} cr</span></>}
                </button>
          ...
          {view === "profile" && (
            <div className="profile-view">
              ...
              <div className="profile-card">
                ...
                <button className="upgrade-btn" onClick={() => showToast("Payment coming soon! 🚀","info")}>⬡ UPGRADE TO PRO — COMING SOON</button>
                <button style={{ background:"none", border:"1px solid rgba(231,76,60,.3)", borderRadius:10, color:"var(--red)", padding:"10px", cursor:"pointer", fontSize:13, fontWeight:500 }} onClick={handleLogout}>Sign Out</button>
              </div>
            </div>
          )}
        </main>
