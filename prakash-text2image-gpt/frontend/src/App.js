...
async function saveToHistory(uid, toolId, outputUrl, prompt) {
  try {
    await addDoc(collection(db, "history"), {
      uid, toolId, outputUrl, prompt, createdAt: serverTimestamp()
    });
  } catch (err) {
...
function UpgradeModal({ user, onClose, onSuccess, showToast }) {
  ...
  const handlePay = async (plan) => {
    ...
    await initiatePayment(
      ...
      (p) => {
        onSuccess(p);
        showToast(`🎉 ${p.name} plan activated! Enjoy your credits!`, "success");
        onClose();
      },
      (err) => {
        showToast(err, "error");
      ...
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-plans">
          {PLANS.map(plan => (
            <div
              key={plan.id}
              className={"modal-plan" + (plan.tag === "MOST POPULAR" ? " modal-plan-popular" : "")}
              style={{ "--pc": plan.color, "--pb": plan.border, "--pd": plan.dim }}
            >
              {plan.tag && (
                <div
                  className="plan-tag"
                  style={{ background: plan.color, color: plan.id === "pro" ? "#fff" : "#000" }}
                >
                  {plan.tag}
                </div>
              )}
...
function LoginScreen({ onLogin }) {
  ...
  useEffect(() => {
    ...
    const stars = Array.from({ length: 120 }, () => ({
      ...
      r: Math.random() * 1.5 + 0.3, o: Math.random(),
    ...
    function draw(t) {
      ...
      [
        [W * 0.2, H * 0.3, W * 0.35, "rgba(0,212,255,0.08)"],
        [W * 0.8, H * 0.7, W * 0.3, "rgba(255,45,120,0.07)"],
        [W * 0.5, H * 0.5, W * 0.4, "rgba(139,92,246,0.05)"]
      ].forEach(([cx, cy, r, c]) => {
  ...
  return (
    <>
      <style>{`
        ...
        .logo-glow-ring { position: absolute; width: 280px; height: 280px; border-radius: 50%; border: 1px solid rgba(0,212,255,0.15); animation: ringPulse 3s ease-in-out infinite; top: 50%; left: 50%; transform: translate(-50%,-50%); }
        .logo-glow-ring2 { position: absolute; width: 240px; height: 240px; border-radius: 50%; border: 1px solid rgba(139,92,246,0.2); animation: ringPulse 3s ease-in-out infinite 1s; top: 50%; left: 50%; transform: translate(-50%,-50%); }
        ...
        .google-btn:hover:not(:disabled) { background: rgba(255,255,255,0.07); border-color: rgba(0,212,255,0.4); box-shadow: 0 0 30px rgba(0,212,255,0.12); transform: translateY(-2px); }
        ...
        .btn-spinner { width: 20px; height: 20px; border: 2px solid rgba(0,212,255,0.2); border-top-color: #00d4ff; border-radius: 50%; animation: spin 0.8s linear infinite; display: inline-block; }
        ...
        .trust-item { display: flex; align-items: center; gap: 6px; font-size: 0.7rem; color: rgba(255,255,255,0.28); }
        ...
        .terms-link { background: none; border: none; color: rgba(0,212,255,0.5); cursor: pointer; font-size: 0.7rem; padding: 0; }
        .plan-badges { display: flex; gap: 8px; margin-top: 28px; justify-content: center; }
        .plan-badge { font-size: 0.68rem; padding: 4px 10px; border-radius: 100px; font-weight: 600; }
...
export default function App() {
  ...
  const creditsLeft = userPlan === "unlimited"
    ? 9999
    : userPlan !== "free"
      ? planCredits
      : FREE_CREDITS_PER_DAY - creditsUsed;
  ...
  const generate = useCallback(async () => {
    ...
    if (needsFile && !inputFile) {
      showToast("Please upload an image first! Click the upload box above.", "error");
      return;
    }
    ...
    if (userPlan !== "free" && userPlan !== "unlimited") {
      setPlanCredits(c => c - activeTool.credits);
    } else {
  ...
  if (authLoading) return (
    <>
      <style>{`
        ...
        @keyframes gradShift2 { 0% { background-position: 0%; } 100% { background-position: 200%; } }
        ...
        .splash-root { position: fixed; inset: 0; background: #03030a; display: flex; flex-direction: column; align-items: center; justify-content: center; z-index: 99999; overflow: hidden; }
        .splash-bg1 { position: absolute; width: 600px; height: 600px; border-radius: 50%; background: radial-gradient(circle, rgba(0,212,255,0.06) 0%, transparent 70%); top: 50%; left: 30%; transform: translate(-50%,-50%); }
        .splash-bg2 { position: absolute; width: 500px; height: 500px; border-radius: 50%; background: radial-gradient(circle, rgba(139,92,246,0.05) 0%, transparent 70%); top: 40%; left: 70%; transform: translate(-50%,-50%); }
        .splash-content { display: flex; flex-direction: column; align-items: center; animation: splashFadeIn 0.8s cubic-bezier(0.16,1,0.3,1) both; position: relative; z-index: 1; }
        ...
        .s-ring { position: absolute; border-radius: 50%; top: 50%; left: 50%; }
        .s-ring1 { width: 160px; height: 160px; border: 1.5px solid rgba(0,212,255,0.3); animation: ring1 2.5s ease-in-out infinite; }
        .s-ring2 { width: 200px; height: 200px; border: 1px solid rgba(139,92,246,0.2); animation: ring2 2.5s ease-in-out infinite 0.3s; }
        .s-arc { position: absolute; width: 140px; height: 140px; border-radius: 50%; border: 2.5px solid transparent; border-top-color: #00d4ff; border-right-color: rgba(0,212,255,0.3); animation: spin 1.2s linear infinite; top: 50%; left: 50%; transform: translate(-50%,-50%); }
        .s-arc2 { position: absolute; width: 155px; height: 155px; border-radius: 50%; border: 1.5px solid transparent; border-bottom-color: #8b5cf6; animation: spin 1.8s linear infinite reverse; top: 50%; left: 50%; transform: translate(-50%,-50%); }
        .splash-logo-img { width: 110px; height: 110px; object-fit: contain; position: relative; z-index: 2; filter: drop-shadow(0 0 30px rgba(0,212,255,0.6)) drop-shadow(0 0 60px rgba(139,92,246,0.4)); animation: logoFloat 3s ease-in-out infinite; }
        .splash-brand { font-family: 'Orbitron', monospace; font-size: 1.7rem; font-weight: 900; letter-spacing: 0.25em; background: linear-gradient(135deg, #00d4ff 0%, #8b5cf6 45%, #ff2d78 80%, #00d4ff 100%); background-size: 200% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; animation: gradShift2 3s linear infinite; margin-top: 28px; }
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .uplan { font-size: 9px; padding: 2px 7px; border-radius: 100px; font-weight: 600; }
        ...
        .profile-plan-badge { display: inline-block; padding: 3px 10px; font-size: 11px; font-weight: 600; letter-spacing: 1px; text-transform: uppercase; margin-top: 4px; border-radius: 6px; }
        ...
        .upgrade-btn-profile { width: 100%; padding: 14px; background: linear-gradient(135deg, var(--cyan), var(--purple), var(--pink)); background-size: 200% auto; border: none; border-radius: 12px; color: #fff; font-family: 'Bebas Neue', sans-serif; font-size: 18px; letter-spacing: 2px; cursor: pointer; transition: all .3s; box-shadow: 0 6px 24px rgba(0,212,255,0.25); animation: gradFlow 4s linear infinite; }
        .upgrade-btn-profile:hover { transform: translateY(-1px); box-shadow: 0 10px 32px rgba(139,92,246,0.4); }
        ...
        /* UPGRADE MODAL */
        .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.85); backdrop-filter: blur(12px); z-index: 9998; display: flex; align-items: center; justify-content: center; padding: 20px; }
        .modal-box { background: #080814; border: 1px solid rgba(139,92,246,0.3); border-radius: 24px; padding: 36px 32px; width: 100%; max-width: 820px; max-height: 90vh; overflow-y: auto; position: relative; animation: modalIn .4s cubic-bezier(0.16,1,0.3,1) both; scrollbar-width: none; }
        .modal-box::-webkit-scrollbar { display: none; }
        .modal-close { position: absolute; top: 16px; right: 16px; background: var(--card2); border: 1px solid var(--border2); border-radius: 8px; color: var(--muted2); font-size: 16px; width: 32px; height: 32px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all .18s; }
        .modal-close:hover { border-color: var(--red); color: var(--red); }
        .modal-header { text-align: center; margin-bottom: 28px; }
        .modal-title { font-family: 'Bebas Neue', sans-serif; font-size: 28px; letter-spacing: 4px; background: linear-gradient(135deg, var(--cyan), var(--purple), var(--pink)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .modal-sub { font-size: 13px; color: var(--muted2); margin-top: 6px; }
        ...
        .modal-plan { background: var(--card); border: 1.5px solid var(--border2); border-radius: 16px; padding: 22px 18px; display: flex; flex-direction: column; gap: 10px; position: relative; transition: all .2s; }
        .modal-plan:hover { border-color: var(--pc, var(--cyan)); box-shadow: 0 8px 32px rgba(0,0,0,.4); }
        .modal-plan-popular { border-color: var(--pb, rgba(139,92,246,0.3)); box-shadow: 0 0 30px rgba(139,92,246,0.1); }
        .plan-tag { position: absolute; top: -1px; left: 50%; transform: translateX(-50%); font-size: 9px; font-weight: 700; letter-spacing: 1.5px; padding: 3px 12px; border-radius: 0 0 8px 8px; }
        .plan-name { font-family: 'Bebas Neue', sans-serif; font-size: 22px; letter-spacing: 2px; margin-top: 8px; }
        .plan-price { display: flex; align-items: baseline; gap: 3px; }
        .plan-rs { font-size: 18px; color: var(--text); font-weight: 600; }
        .plan-amount { font-family: 'Bebas Neue', sans-serif; font-size: 42px; line-height: 1; color: var(--text); }
        .plan-period { font-size: 12px; color: var(--muted2); }
        .plan-credits { font-size: 13px; font-weight: 600; margin-bottom: 4px; }
        .plan-features { display: flex; flex-direction: column; gap: 6px; flex: 1; }
        .plan-feature { font-size: 12px; color: var(--muted2); display: flex; align-items: center; gap: 6px; }
        .plan-btn { width: 100%; padding: 12px; border: none; border-radius: 10px; font-family: 'Bebas Neue', sans-serif; font-size: 16px; letter-spacing: 1.5px; cursor: pointer; transition: all .2s; display: flex; align-items: center; justify-content: center; gap: 8px; margin-top: auto; }
        .plan-btn:hover:not(:disabled) { transform: translateY(-1px); filter: brightness(1.1); }
        .plan-btn:disabled { opacity: .6; cursor: not-allowed; }
        .btn-spin { width: 16px; height: 16px; border: 2px solid rgba(255,255,255,0.3); border-top-color: #fff; border-radius: 50%; animation: spin .8s linear infinite; display: inline-block; }
        .modal-footer { display: flex; justify-content: center; gap: 24px; margin-top: 24px; padding-top: 20px; border-top: 1px solid var(--border); flex-wrap: wrap; }
        .modal-footer span { font-size: 12px; color: var(--muted2); }
        ...
        <aside className={"sidebar" + (sidebarOpen ? " open" : "")}>
          ...
          <div className="credits-bar">
            <div className="credits-row">
              ...
              <span className="credits-count">
                {userPlan === "unlimited" ? "∞" : creditsLeft}
                {userPlan === "free" ? ` / ${FREE_CREDITS_PER_DAY}` : ""}
              </span>
            </div>
            {userPlan !== "unlimited" && (
              <div className="credits-track">
                <div
                  ...
                  style={{
                    width: userPlan === "free"
                      ? (creditsLeft / FREE_CREDITS_PER_DAY * 100) + "%"
                      : Math.min((creditsLeft / 2000) * 100, 100) + "%"
                  }}
                />
              </div>
            )}
          ...
          <div className="sidebar-views">
            ...
            >
              ...
              <span className="view-lbl">History</span>
            </div>
            <div
              className={"view-btn" + (view === "profile" ? " active" : "")}
              onClick={() => { setView("profile"); setSidebarOpen(false); }}
            >
              <span className="view-icon">◯</span>
              <span className="view-lbl">Profile</span>
            </div>
          </div>
          {userPlan === "free" && (
        ...
        <main className="main">
          <div className="mobile-topbar">
            ...
            <span style={{ fontSize: 11, color: "var(--muted2)" }}>
              {userPlan === "unlimited" ? "∞" : creditsLeft}cr
            </span>
          </div>
          ...
          {view === "create" && (
            <div className="workspace">
              <div className="controls">
                  ...
                  <div className="ctrl-section">
                    ...
                    {!inputFile && (
                      <div style={{
                        ...
                        background: "rgba(231,76,60,0.06)", borderRadius: 8,
                  ...
                  <div className="ctrl-section">
                    ...
                    <div className="style-grid">
                      {STYLES.map((s, i) => (
                        <div
                          key={i}
                          className={"style-pill" + (style === i ? " active" : "")}
                ...
                <button
                  className="gen-btn"
                  disabled={loading || (needsFile && !inputFile)}
                  onClick={generate}
                >
                  {loading ? (
                    <>
                    ...
                    <>
                      <span>{activeTool.icon} Generate</span>
                ...
                {creditsLeft < activeTool.credits && !loading && (
                  <div
                    style={{ fontSize: 11, color: "var(--red)", textAlign: "center", cursor: "pointer" }}
                      ...
                      <div className="audio-player">
                        ...
                        <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
                          <button
                            ...
                            style={{ background: "var(--purple-dim)", borderColor: "rgba(139,92,246,0.3)", color: "var(--purple)" }}
                            onClick={() => {
                              window.speechSynthesis.cancel();
                              const u = new SpeechSynthesisUtterance(result.text);
                              u.rate = 0.9; u.pitch = 1; u.volume = 1;
                              const v = window.speechSynthesis.getVoices().find(v => v.lang.startsWith("en") && v.name.includes("Google"))
                        ...
                        <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 10 }}>
                          💡 Uses browser built-in speech engine
                        </div>
                      </div>
                    )}
                ...
                {history.length > 0 && (
                  <button
                    style={{
                      ...
                      borderRadius: 8, color: "var(--red)", fontSize: 12, padding: "6px 14px",
                ...
                <div className="history-grid">
                  {history.map(h => (
                    <div key={h.id} className="history-card" style={{ position: "relative" }}>
                      <div
                        onClick={() => {
                          setResult({ type: "image", url: h.outputUrl });
                          setActiveTool(TOOLS.find(t => t.id === h.toolId) || TOOLS[0]);
                          setView("create");
                        }}
                      >
                        <img
              ...
              <div className="profile-card">
                ...
                <div className="stats-grid">
                  <div className="stat-card">
                    <div className="stat-value">{userPlan === "unlimited" ? "∞" : creditsLeft}</div>
                    <div className="stat-label">Credits Remaining</div>
                  </div>
                  <div className="stat-card">
                    <div className="stat-value" style={{ color: "var(--purple)" }}>{userPlan === "free" ? creditsUsed : "—"}</div>
                    <div className="stat-label">Used Today</div>
                  </div>
                  <div className="stat-card">
                    <div className="stat-value" style={{ color: "var(--pink)" }}>
                      {userPlan === "free" ? FREE_CREDITS_PER_DAY : planInfo?.credits === 99999 ? "∞" : planInfo?.credits || "—"}
                    </div>
                    <div className="stat-label">{userPlan === "free" ? "Daily Free Credits" : "Plan Credits"}</div>
                  </div>
                  <div className="stat-card">
                ...
                {userPlan !== "free" && (
                  <div style={{
                    ...
                    ✅ You are on the{" "}
                    <span style={{ color: planInfo?.color, fontWeight: 600 }}>{userPlan.toUpperCase()}</span>
                    {" "}plan — Enjoy your credits!
                  </div>
                )}
                <button
                  style={{
                    background: "none", border: "1px solid rgba(231,76,60,.25)",
                    borderRadius: 10, color: "var(--red)", padding: 10,
