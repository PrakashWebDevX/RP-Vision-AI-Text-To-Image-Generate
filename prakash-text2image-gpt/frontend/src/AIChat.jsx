    ...
    <div style={{flex:1,display:"flex",height:"100%",overflow:"hidden",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{width:230,display:"flex",flexDirection:"column",borderRight:"1px solid rgba(255,255,255,0.05)",background:"#07070f",flexShrink:0}}>
        ...
        <div style={{padding:"10px 10px 6px",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
          <div style={{fontSize:9,color:"#44445a",letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>Model</div>
          <div style={{position:"relative"}}>
            <button onClick={()=>setShowModels(p=>!p)} style={{
              width:"100%",background:"#0d0d1a",border:"1px solid rgba(255,255,255,0.08)",
              borderRadius:9,padding:"8px 10px",color:"#eeeef5",fontSize:12,cursor:"pointer",
              display:"flex",alignItems:"center",gap:8,fontFamily:"'DM Sans',sans-serif",transition:"border-color .2s"
            }} onMouseEnter={e=>e.currentTarget.style.borderColor="rgba(139,92,246,0.4)"}
               onMouseLeave={e=>e.currentTarget.style.borderColor="rgba(255,255,255,0.08)"}>
              <span>{model.icon}</span>
              <span style={{flex:1,textAlign:"left",fontWeight:500}}>{model.name}</span>
              <span style={{fontSize:9,background:model.color+"22",color:model.color,padding:"2px 6px",borderRadius:100,border:`1px solid ${model.color}44`}}>{model.badge}</span>
              <span style={{color:"#44445a",fontSize:10}}>{showModels?"▴":"▾"}</span>
            </button>
            {showModels && (
              <div style={{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,
                background:"#0d0d1a",border:"1px solid rgba(255,255,255,0.1)",
                borderRadius:10,overflow:"hidden",zIndex:99,boxShadow:"0 12px 32px rgba(0,0,0,0.6)"}}>
                {MODELS.map(m=>(
                  <div key={m.id} onClick={()=>{ setModel(m); setShowModels(false); }}
                    style={{padding:"11px 12px",cursor:"pointer",display:"flex",gap:10,alignItems:"center",
        ...
        <div style={{flex:1,overflowY:"auto",padding:"8px 6px",scrollbarWidth:"none"}}>
          ...
          {[...sessions].reverse().map(s=>(
            <div key={s.id} onClick={()=>switchChat(s.id)}
              ...
              onMouseEnter={e=>{ if(s.id!==activeId) e.currentTarget.style.background="rgba(255,255,255,0.03)"; e.currentTarget.querySelector('.del-btn').style.opacity=1; }}
              onMouseLeave={e=>{ if(s.id!==activeId) e.currentTarget.style.background="transparent"; e.currentTarget.querySelector('.del-btn').style.opacity=0; }}>
              ...
              <button className="del-btn" onClick={(e)=>deleteChat(s.id,e)}
                style={{background:"none",border:"none",color:"#e74c3c",fontSize:11,cursor:"pointer",opacity:0,transition:"opacity .15s",padding:"2px 4px",flexShrink:0,borderRadius:4}}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
        ...
        <div style={{padding:"14px 24px",borderBottom:"1px solid rgba(255,255,255,0.05)",display:"flex",alignItems:"center",justifyContent:"space-between",background:"rgba(7,7,15,0.9)",backdropFilter:"blur(12px)",flexShrink:0}}>
          <div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,letterSpacing:3,background:"linear-gradient(135deg,#00d4ff,#8b5cf6,#ff2d78)",backgroundSize:"200%",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>
              AI ASSISTANT
            </div>
            <div style={{fontSize:11,color:"#7777a0",marginTop:1}}>
              {model.icon} {model.name} · Ask me anything — coding, questions, jokes & more!
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:6,background:"#0d0d1a",border:"1px solid rgba(255,255,255,0.08)",padding:"5px 12px",borderRadius:100}}>
            ...
            <span style={{fontSize:11,color:"#7777a0"}}>Online · Free</span>
          </div>
        </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"24px",display:"flex",flexDirection:"column",gap:18,scrollbarWidth:"none"}}>
          {msgs.length===0 && (
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:24,paddingTop:20,maxWidth:680,margin:"0 auto",width:"100%"}}>
              {/* Welcome */}
              <div style={{textAlign:"center"}}>
                <div style={{width:80,height:80,borderRadius:"50%",background:"linear-gradient(135deg,rgba(0,212,255,0.1),rgba(139,92,246,0.1))",border:"2px solid rgba(139,92,246,0.25)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:36,margin:"0 auto 16px"}}>◈</div>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,letterSpacing:4,color:"#eeeef5"}}>HOW CAN I HELP YOU?</div>
                <div style={{fontSize:13,color:"#7777a0",marginTop:6,lineHeight:1.7}}>I can help with coding, answer questions, tell jokes, explain concepts<br/>and pretty much everything else!</div>
              </div>
              {/* Quick Prompts */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,width:"100%"}}>
                {QUICK.map((q,i)=>(
                  <button key={i} onClick={()=>send(q.text)}
                    style={{background:"#0d0d1a",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,padding:"11px 14px",color:"#7777a0",fontSize:12.5,cursor:"pointer",textAlign:"left",transition:"all .2s",fontFamily:"'DM Sans',sans-serif",lineHeight:1.5,display:"flex",gap:8,alignItems:"flex-start"}}
          ...
          {msgs.map((m,i)=>(
            <div key={i} style={{display:"flex",gap:12,flexDirection:m.role==="user"?"row-reverse":"row",alignItems:"flex-start",maxWidth:"85%",alignSelf:m.role==="user"?"flex-end":"flex-start"}}>
              {/* Avatar */}
              <div style={{
                width:36,height:36,borderRadius:"50%",flexShrink:0,
                display:"flex",alignItems:"center",justifyContent:"center",
                fontSize:m.role==="user"?13:18,fontWeight:700,
              ...
              <div style={{
                ...
                background:m.role==="user"?"linear-gradient(135deg,rgba(0,212,255,0.12),rgba(139,92,246,0.12))":"#0d0d1a",
                ...
                {m.role==="assistant" && (
                  <div style={{marginTop:10,paddingTop:8,borderTop:"1px solid rgba(255,255,255,0.05)",display:"flex",gap:12}}>
                    <button onClick={()=>copy(m.content,i)}
                      style={{background:"none",border:"none",color:copied===i?"#2ecc71":"#44445a",fontSize:11,cursor:"pointer",padding:0,transition:"color .2s",display:"flex",alignItems:"center",gap:4}}
          ...
          {loading && (
            <div style={{display:"flex",gap:12,alignItems:"flex-start",alignSelf:"flex-start"}}>
              <div style={{width:36,height:36,borderRadius:"50%",background:"linear-gradient(135deg,rgba(139,92,246,0.15),rgba(255,45,120,0.1))",border:"1px solid rgba(139,92,246,0.25)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{model.icon}</div>
              <div style={{background:"#0d0d1a",border:"1px solid rgba(255,255,255,0.06)",borderRadius:"4px 16px 16px 16px",padding:"14px 18px",display:"flex",gap:5,alignItems:"center"}}>
                {[0,150,300].map((d,i)=>(
                  <div key={i} style={{width:7,height:7,borderRadius:"50%",background:"#8b5cf6",animation:`typing-dot 1.2s ease-in-out ${d}ms infinite`}}/>
                ))}
              </div>
            </div>
          )}
        ...
        <div style={{padding:"14px 20px 16px",borderTop:"1px solid rgba(255,255,255,0.05)",background:"rgba(7,7,15,0.95)",flexShrink:0}}>
          <div style={{
            ...
            borderRadius:16,padding:"12px 14px",transition:"border-color .2s"
          }}
            onFocus={()=>{}} onBlur={()=>{}}>
            <textarea ref={inputRef} value={input}
              onChange={e=>{ setInput(e.target.value); e.target.style.height="auto"; e.target.style.height=Math.min(e.target.scrollHeight,140)+"px"; }}
              onKeyDown={e=>{ if(e.key==="Enter"&&!e.shiftKey){ e.preventDefault(); send(); } }}
              placeholder="Ask anything — code, jokes, questions, news... (Enter to send, Shift+Enter for new line)"
              style={{flex:1,background:"none",border:"none",color:"#eeeef5",fontSize:13.5,resize:"none",outline:"none",fontFamily:"'DM Sans',sans-serif",lineHeight:1.6,height:22,maxHeight:140,overflowY:"auto",padding:0,scrollbarWidth:"none"}}
            />
            <button onClick={()=>send()} disabled={loading||!input.trim()}
              style={{
                width:40,height:40,borderRadius:11,border:"none",cursor:input.trim()&&!loading?"pointer":"not-allowed",
                background:input.trim()&&!loading?"linear-gradient(135deg,#00d4ff,#8b5cf6)":"rgba(255,255,255,0.05)",
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:8,padding:"0 2px"}}>
            ...
            <span style={{fontSize:10,color:"#44445a"}}>Powered by <span style={{color:model.color}}>{model.name}</span> · Free ✓</span>
          </div>
        </div>
      </div>
